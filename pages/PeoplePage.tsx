        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            {isAiConfigured && (
                <div>
                    ...
                    {ocrError && <p className="text-red-500 text-sm mt-2 text-center select-text cursor-text">{ocrError}</p>}
                    {ocrWarning && <p className="text-amber-700 bg-amber-50 border border-amber-200 text-sm mt-2 text-center p-2 rounded-md select-text cursor-text">{ocrWarning}</p>}
                </div>
            )}
            <div>
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700">Номер НРС</label>
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">НРС</th>
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        {people.length > 0 ? people.map(person => (
                            <tr key={person.id} className="hover:bg-slate-50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 select-text cursor-text">{person.name}</td>
                                <td className="px-6 py-4 text-sm text-slate-600 max-w-sm whitespace-normal select-text cursor-text">{person.position}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 select-text cursor-text">{person.organization || <span className="text-slate-400 italic">Не указана</span>}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 select-text cursor-text">{person.nrs || <span className="text-slate-400 italic">-</span>}</td>
